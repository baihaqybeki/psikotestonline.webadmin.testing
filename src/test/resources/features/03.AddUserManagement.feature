...
Scenario: Add existing user
		...
		And click admin menu
		And click add button
		And fill user data form with full name with "admin" as privileges and "developer" as supervisor
		And click button save
		And click yes button on confirmation dialog box
		Then user appear popup message "Duplicate entry 'newuser' for key 'admin.username'"
